...
from sgl_jax.srt.models.qwen2_vl import Qwen2VLForConditionalGeneration
...
MAX_PIXELS = 1280 * 720 * IMAGE_FACTOR  # 假设最大图像像素为1280x720，可根据实际需求调整
MAX_RATIO = 200
RESIZE_RESAMPLE = Image.BILINEAR  # 使用常见的双线性插值作为默认值
VIDEO_TOTAL_PIXELS = int(128000 * 28 * 28 * 0.9)  # 保持原计算逻辑但移除环境变量依赖
...
async def preprocess_video(
    ...
    # 转换为JAX数组并调整维度顺序 (TCHW)
    video = jax.device_put(video_np)
    video = jnp.transpose(video, (0, 3, 1, 2))  # 从THWC转换为TCHW
    ...
    video = jax.image.resize(
        video,
        (nframes, 3, resized_height, resized_width),
        method="bilinear"
    )
        ...
        if self.model_type == "qwen3_omni_moe":
            ...
            if audio_item:
                audio_feature_lengths = jnp.sum(
                    audio_item.feature_attention_mask, axis=1
                )
