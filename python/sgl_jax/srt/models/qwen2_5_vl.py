...
class Qwen2_5VLAttention(nnx.Module):
    ...
    def __call__(
        ...
        k = k.reshape(
            (-1, self.kv_head_num, self.head_dim),
...
class Qwen2_5VLDecoderLayer(nnx.Module):
    ...
    def __call__(
        ...
        if hidden_states.ndim != 3:
            logger.warning(f"hidden_states 期望3D，实际{hidden_states.ndim}D，形状: {hidden_states.shape}")
            # 尝试从批次信息推断维度
        ...
        if attn_output.ndim != 3:
            logger.warning(f"attn_output 期望3D，实际{attn_output.ndim}D，形状: {attn_output.shape}")
...
class Qwen2_5VLModel(nnx.Module):
    def __init__(
        ...
        if self.has_vision:
            ...
            vision_config.rms_norm_eps = getattr(vision_config, "rms_norm_eps", 
                             getattr(config, "rms_norm_eps", 1e-6))
            vision_config.hidden_size = getattr(vision_config, "hidden_size", 
                                              config.hidden_size)
            vision_config.intermediate_size = getattr(vision_config, "intermediate_size", 
                                                    config.intermediate_size)
            vision_config.num_attention_heads = getattr(vision_config, "num_attention_heads", 
                                                      config.num_attention_heads)
            vision_config.num_key_value_heads = getattr(vision_config, "num_key_value_heads", 
                                                       config.num_key_value_heads)
            vision_config.max_position_embeddings = getattr(vision_config, "max_position_embeddings", 
                                                          1024)
    ...
    def __call__(
        ...
        # 处理视觉输入
        if self.has_vision and hasattr(forward_batch, 'vision_inputs') and forward_batch.vision_inputs is not None:
            vision_inputs = forward_batch.vision_inputs
            # 视觉输入处理（确保输入维度正确）
            if vision_inputs.ndim != 3:
                raise ValueError(f"视觉输入期望3D，实际{vision_inputs.ndim}D: {vision_inputs.shape}")
                
            vision_emb = self.vision_embed_tokens(vision_inputs)
            # 添加位置嵌入（确保广播维度正确）
            if vision_emb.shape[1] != self.vision_pos_embed.shape[1]:
                logger.warning(f"视觉嵌入与位置嵌入长度不匹配: {vision_emb.shape[1]} vs {self.vision_pos_embed.shape[1]}")
            ...
            for vision_layer in self.vision_layers:
                vision_emb, vision_residual, _, _ = vision_layer(
                    positions=getattr(forward_batch, "vision_positions", jnp.arange(vision_emb.shape[1])),
            ...
            if hidden_states.shape[0] != vision_emb.shape[0]:
                raise ValueError(f"文本与视觉批次不匹配: {hidden_states.shape[0]} vs {vision_emb.shape[0]}")
